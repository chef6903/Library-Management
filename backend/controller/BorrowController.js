  ...
  try {
    ...
    if (isOverdue || conditionCounts.damaged > 0 || conditionCounts.lost > 0) {
      ...
      if (conditionCounts.damaged > 0) {
        const damagedFine =
          borrowRequest.bookId.price * 0.3 * conditionCounts.damaged;
...
exports.getReturnHistory = async (req, res) => {
  try {
    ...
    const borrowRecords = await BorrowRecord.find({ status: "returned" })
      ...
      .populate(
        "bookId",
        "title isbn author publisher publishYear description price image"
      )
      .skip(skip)
      .limit(limit)
      .sort({ returnDate: -1 });
...
exports.getReturnHistoryByUser = async (req, res) => {
  try {
    ...
    const borrowRecords = await BorrowRecord.find(query)
      ...
      .populate(
        "bookId",
        "title isbn author publisher publishYear description price image bookcopies"
      )
      .skip(skip)
...
exports.confirmBookPickup = async (req, res) => {
  try {
    ...
    if (borrowRecord.status !== "pendingPickup") {
      return res
        .status(400)
        .json({ message: "Borrow is not in pending pickup state" });
    }
