...
export const acceptBorrowRequest = async (borrowId) => {
  try {
    const response = await api.post(
      `/borrows/accept-borrow-request/${borrowId}`
    );
    return response.data;
  } catch (error) {
...
export const returnBook = async (borrowId, data) => {
  try {
    const response = await api.post(`/borrows/return-book/${borrowId}`, data, {
      headers: {
        Authorization: `Bearer ${localStorage.getItem("token")}`,
      },
    });
...
export const getAllReturnHistory = async (page = 1, limit = 10) => {
  try {
    ...
    console.error(
      "Error fetching all return history:",
      error.response?.data || error.message
    );
    throw error;
  }
...
export const getBorrowedBooksByUser = async (
  ...
  try {
    ...
    console.error(
      "Error fetching borrowed books by user:",
      error.response?.data || error.message
    );
    throw error;
  }
...
export const cancelBorrowRequest = async (borrowId) => {
  try {
    ...
    console.error(
      "Error cancelling borrow request:",
      error.response?.data || error.message
    );
    throw error;
  }
